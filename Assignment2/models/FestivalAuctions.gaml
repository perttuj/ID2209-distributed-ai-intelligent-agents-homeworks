...
species FestivalGuest skills:[moving, fipa]
{
	...
	{
		message requestFromInitiator <- informs[0];
		string informType <- requestFromInitiator.contents[0];
		...
		if (informType = "start_auction") {
			...
			if joinAuction = false and THIRST > 10000 and HUNGER > 10000 and money > 0 {
				...
				do agree with: (message: requestFromInitiator, contents: [self.name + ': I will']);
				joinAuction <- true;
			} else {
				// write self.name + ": refusing invitation, money: " + money;
				do refuse with: (message: requestFromInitiator, contents: [self.name + ': I won\'t']);
				joinAuction <- false;
			}
...
species Initiator skills: [fipa]
{
	...
	reflex startNewAuction when: started = false and sold = true {
		...
		if (flip(0.001)) {
			// setting sold to false will trigger our other reflex,
	...
	reflex notifyAuctionStart when: initiated = false and sold = false {
		...
		initiated <- true;
		do start_conversation (to :: allGuests, protocol :: 'fipa-request', performative :: 'inform', contents :: ['start_auction']);
	}
	...
	reflex start_auction when: initiated = true and started = false and !(empty(buyers))
	...
	reflex read_accepts when: started = true and !(empty(proposes)) {
		int proposalsSize <- length(proposes);
		bool accepted <- false;
		loop proposeMsg over: proposes {
			if (accepted = false) {
				accepted <- true;
				write self.name + ": accepting proposal: " + proposeMsg;
	...
	reflex receiveRefuseMessages when: started = true and !(empty(refuses)) {
		...
		if (lengthOfRefuses = length(buyers)) {
			...
			write "lowering price to: " + tmpPrice;
			do start_conversation with: [ to :: list(buyers), protocol :: 'fipa-contract-net', performative :: 'cfp', contents :: ['price', tmpPrice] ];
		}
	}
